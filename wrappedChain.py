...
class wrappedChain(dict) : 
    ...
    def calcDependencies(self) : return dict([(node.name(),node.source.keys() if hasattr(node.source,"keys") else set()) for node in filter(lambda n: hasattr(n,"source"), self.__activeNodes.values()) ])
