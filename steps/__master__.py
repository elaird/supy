...
class master(supy.analysisStep):
    ...
    def select(self, eventVars):
        ...
        if weight is None:
            ...
            self.increment(False, 1-weight)
        return weight is not None
    # 'None' weight counts toward the total cross section with weight = 1,
    # but is not processed further.  This is useful for sub-sampling
    # and pthat-filtering.

    def endFunc(self, chains):
        self.book.fill(0.0, "nJobsHisto", 1, -0.5, 0.5,
                       title=";dummy axis;N_{jobs}",
                       w=1.0)

        if self.xs:
            self.book.fill(0.0, "xsHisto", 1, -0.5, 0.5,
                           title=";dummy axis;#sigma (pb)",
                           w=self.xs)

        if self.xsPostWeights:
            self.book.fill(0.0, "xsPostWeightsHisto", 1, -0.5, 0.5,
                           title=";dummy axis;#sigma (pb)",
                           w=self.xsPostWeights)

        if self.lumi:
            if self.lumiWarn:
                title = "WARNING: lumi value is probably wrong!"
            else:
                title = ""
            title += ";dummy axis;integrated luminosity (pb^{-1})"
            self.book.fill(0.0, "lumiHisto", 1, -0.5, 0.5,
                           title=title,
                           w=self.lumi)
    ...
    def mergeFunc(self, products):
        ...
        hAdd = supy.utils.getCommandOutput(cmd)
