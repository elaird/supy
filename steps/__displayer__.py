...
class displayer(supy.analysisStep):
    def reset(self):
        self.canvas.Clear()

    def display(self, eventVars):
        """Implement drawing etc. in self.display.
        It may be necessary to return ROOT objects
    ...
    def setup(self, chain, fileDir):
        ...
        self.canvas = supy.utils.canvas("canvas")
    ...
    def mergeFunc(self, products):
        def pdfFromRoot(listOfInFileNames, outFileName):
            if not len(listOfInFileNames):
                return
            options = "pdf"
            dummyCanvas = supy.utils.canvas("display")
            ...
            for inFileName in listOfInFileNames:
                ...
                for key in keys:
                    ...
                    if someObject.ClassName() != "TCanvas":
                        print "Warning: found an non-TCanvas object" + \
                              " in the display root file"
        ...
        pdfFromRoot(products["outputFileName"],
                    self.outputFileName.replace(".root", ".pdf"),
                    )
