...
class topPrinter(analysisStep) :
    ...
    def uponAcceptance (self,ev) :
        ...
        print
        for i in iQs+[iTop,iTbar]+iGs :
            print '\t'.join(str(d)[:5] for d in ["%d(%s)"%(i,labels[ids[i]]), mom[i], p4s[i].pt(), '-', p4s[i].eta(), p4s[i].phi()])
        print
        for i,j in itertools.combinations(iQs,2) :
            if ids[i]*ids[j]>0 : continue
            p4 = p4s[i]+p4s[j]
            print '\t'.join(str(d)[:5] for d in ["[%s,%s]"%(i,j), '-', p4.pt(), p4.M(), p4.eta(), p4.phi()])
